...
[ 임베딩 대상 ]
  - boardlife 리뷰 : review_text → faiss_boardlife_reviews.index
...
def load_boardlife_reviews(path="data/boardlife_reviews_final.csv"):
    """
    보드라이프 리뷰 데이터 로드.
    임베딩 텍스트 : review_text
    메타데이터    : rank, title, rating, source
    """
    df = pd.read_csv(path)

    # 임베딩 텍스트: review_text만 사용
    texts = df["review_text"].fillna(" ").tolist()

    # 메타데이터 컬럼 목록
    meta_cols = ["rank", "title", "rating", "source"]
...
def main():
    ...
    # ── 보드라이프 리뷰 ─────────────────────────
    print("\n📂 boardlife_reviews_final.csv 로드 중...")
    texts, meta = load_boardlife_reviews()
    ...
    save_index(
        embeddings, meta,
        index_path="faiss_boardlife_reviews.index",
        meta_path="faiss_boardlife_reviews_meta.json"
    )

    print("\n🎉 임베딩 완료!")
    print("   faiss_boardlife_reviews.index / faiss_boardlife_reviews_meta.json")
