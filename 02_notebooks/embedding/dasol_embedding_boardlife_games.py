...
[ 임베딩 대상 ]
  - boardlife 소개 : description → faiss_boardlife_stats.index
...
def load_boardlife_stats(path="data/boardlife_games_stats_final.csv"):
    """
    보드라이프 소개 데이터 로드.
    임베딩 텍스트 : description
    메타데이터    : rank, title, title_eng, age, weight, designer, artist,
                   type, category, mechanism, image, avg_rating, category_rank,
                   min_players, max_players, best_players, recommend_players,
                   min_time, max_time, source
    """
    df = pd.read_csv(path)

    # 임베딩 텍스트: description만 사용
    # fillna(" "): 빈 값이 있으면 공백으로 대체 (OpenAI API 에러 방지)
...
def main():
    ...
    # ── 보드라이프 소개 ─────────────────────────
    print("\n📂 boardlife_games_stats_final.csv 로드 중...")
    texts, meta = load_boardlife_stats()
    ...
    save_index(
        embeddings, meta,
        index_path="faiss_boardlife_stats.index",
        meta_path="faiss_boardlife_stats_meta.json"
    )

    print("\n🎉 임베딩 완료!")
    print("   faiss_boardlife_stats.index / faiss_boardlife_stats_meta.json")
